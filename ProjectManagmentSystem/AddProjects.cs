...
namespace ProjectManagmentSystem
{
    public partial class AddProjects : Form
    {
        ...
        {
            string query = "INSERT INTO projects(FileName,Description,StartDate,CompletedDate,DateAdded,EmployeeId,StatusId) VALUES (@FileName,@Description,@StartDate,@CompletedDate,@DateAdded,@EmployeeId,@StatusId); SELECT LAST_INSERT_ID()";
            MySqlCommand cmd = new MySqlCommand(query, conn);
            cmd.Parameters.AddWithValue("@FileName", bunifuTextBox1.Text);
        ...
        private void AddProjects_Load(object sender, EventArgs e)
        {
            ...
            MySqlCommand statusId = new MySqlCommand("SELECT Id,StatusName FROM status", conn);
            MySqlDataReader readerStatusId = statusId.ExecuteReader();

            while (readerStatusId.Read())
            {
                string id = readerStatusId.GetString("Id");
